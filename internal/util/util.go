...
func IsConfigChanged(src, dest string) (bool, error) {
	...
	if d.Uid != s.Uid {
		log.Print(fmt.Sprintf("%s has UID %d should be %d", dest, d.Uid, s.Uid))
	}
	if d.Gid != s.Gid {
		log.Print(fmt.Sprintf("%s has GID %d should be %d", dest, d.Gid, s.Gid))
	}
	if d.Mode != s.Mode {
		log.Print(fmt.Sprintf("%s has mode %s should be %s", dest, os.FileMode(d.Mode), os.FileMode(s.Mode)))
	}
	if d.Md5 != s.Md5 {
		log.Print(fmt.Sprintf("%s has md5sum %s should be %s", dest, d.Md5, s.Md5))
	}
