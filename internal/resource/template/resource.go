...
import (
	...
	util "github.com/ltkh/confd/internal/util"
...
func NewTemplateResource(path string, config Config) (*TemplateResource, error) {
	...
	log.Print("Loading template resource from " + path)
...
func (t *TemplateResource) setVars() error {
	var err error
	log.Print("Retrieving keys from store")
	log.Print("Key prefix set to " + t.Prefix)

	result, err := t.storeClient.GetValues(util.AppendPrefix(t.Prefix, t.Keys))
	if err != nil {
		return err
	}
	log.Print("Got the following map from store: %v", result)
...
func (t *TemplateResource) createStageFile() error {
	log.Print("Using source template " + t.Src)

	if !util.IsFileExist(t.Src) {
		return errors.New("Missing template: " + t.Src)
	}

	log.Print("Compiling source template " + t.Src)
...
func (t *TemplateResource) sync() error {
	...
	if t.keepStageFile {
		log.Print("Keeping staged file: " + staged)
	} else {
		defer os.Remove(staged)
	}

	log.Print("Comparing candidate config to " + t.Dest)
	ok, err := util.IsConfigChanged(staged, t.Dest)
	if err != nil {
		log.Print(err.Error())
	}
	if t.noop {
		log.Print("Noop mode enabled. " + t.Dest + " will not be modified")
		return nil
	}
	if ok {
		log.Print("Target config " + t.Dest + " out of sync")
		if !t.syncOnly && t.CheckCmd != "" {
			if err := t.check(); err != nil {
				return errors.New("Config check failed: " + err.Error())
			}
		}
		log.Print("Overwriting target config " + t.Dest)
		err := os.Rename(staged, t.Dest)
		if err != nil {
			if strings.Contains(err.Error(), "device or resource busy") {
				log.Print("Rename failed - target is likely a mount. Trying to write instead")
		...
		log.Print("Target config " + t.Dest + " has been updated")
	} else {
		log.Print("Target config " + t.Dest + " in sync")
	}
	return nil
}
...
func runCommand(cmd string) error {
	...
	if err != nil {
		log.Print(fmt.Sprintf("%q", string(output)))
		return err
	}
	log.Print(fmt.Sprintf("%q", string(output)))
	return nil
