...
func getTemplateResources(config Config) ([]*TemplateResource, error) {
	...
	log.Print("Loading template resources from confdir " + config.ConfDir)
	if !util.IsFileExist(config.ConfDir) {
		log.Print(fmt.Sprintf("Cannot load template resources: confdir '%s' does not exist", config.ConfDir))
		return nil, nil
	}
	...
	if len(paths) < 1 {
		log.Print("Found no templates")
	}

	for _, p := range paths {
		log.Print(fmt.Sprintf("Found template: %s", p))
